...
import UserFilter from "./supply-filter";
...
function Actions({ entitiy }: { entitiy: Supply }) {
  const [open, setOpen] = useState(false);
  const { confirmDialog } = useConfirmDialog();
  const fetchDelete = useDeleteSupplyService();
  ...
  const handleDelete = async () => {
    const isConfirmed = await confirmDialog({
      title: tSupplies("admin-panel-supplies:confirm.delete.title"),
      message: tSupplies("admin-panel-supplies:confirm.delete.message"),
    });
    ...
    if (isConfirmed) {
      ...
      let filter: SupplyFilterType | undefined = undefined;
      let sort: SupplySortType | undefined = {
        order: SortEnum.ASC,
        orderBy: "id",
      };
      ...
      const previousData = queryClient.getQueryData<
        InfiniteData<{ nextPage: number; data: Supply[] }>
      >(suppliesQueryKeys.list().sub.by({ sort, filter }).key);

      await queryClient.cancelQueries({
        queryKey: suppliesQueryKeys.list().key,
      });
  ...
  const mainButton = (
    <Button
      ...
      variant="contained"
      LinkComponent={Link}
      href={`/admin-panel/supplies/edit/${entitiy.id}`}
    >
      {tSupplies("admin-panel-supplies:actions.edit")}
    </Button>
  );
              ...
              <ClickAwayListener onClickAway={handleClose}>
                <MenuList id="split-button-menu" autoFocusItem>
                  {canDelete && (
                    ...
                    >
                      {tSupplies("admin-panel-supplies:actions.delete")}
                    </MenuItem>
                  )}
                </MenuList>
              </ClickAwayListener>
...
function Supplies() {
  ...
  const { data, hasNextPage, isFetchingNextPage, fetchNextPage } =
    useSupplyListQuery({ filter, sort: { order, orderBy } });
            ...
            fixedHeaderContent={() => (
              <>
                <TableRow>
                  ...
                  >
                    {tSupplies("admin-panel-supplies:table.column1")}
                  </TableSortCellWrapper>
                  <TableCell style={{ width: 200 }}>
                    {tSupplies("admin-panel-supplies:table.column2")}
                  </TableCell>
                  <TableSortCellWrapper
                    orderBy={orderBy}
                    order={order}
                    ...
                    handleRequestSort={handleRequestSort}
                  >
                    {tSupplies("admin-panel-supplies:table.column3")}
                  </TableSortCellWrapper>
